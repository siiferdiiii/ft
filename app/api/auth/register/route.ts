  ...
  try {
    ...
    if (admin) {
      try {
        ...
        console.warn("Supabase admin registration fallback:", err);
      }
    }

    // 2. Cek apakah URL Supabase valid (bukan dummy/placeholder example.supabase.co)
    ...
    if (isSupabaseConfigured) {
      try {
        ...
        if (error) {
          // Jika error fetch / network (misal status 0 atau host belum terhubung)
          if (
            error.status === 0 ||
            error.name === "AuthRetryableFetchError" ||
            error.message?.toLowerCase().includes("fetch failed")
          ) {
            console.warn("Supabase fetch failed (network unreachable), proceeding with database fallback:", error.message);
          } else if (
        ...
        console.warn("Supabase auth offline / fetch error, fallback to database:", supabaseErr);
      }
    }

    // 3. Fallback database registration jika Supabase tidak aktif / fetch failed / rate limit
